...
function sourceCard(kind: SourceKind): string {
  ...
  return `<article class="source-card ${table ? 'is-loaded' : ''}" data-kind="${kind}">
    ...
    ${table ? `<div class="file-proof"><strong>${esc(table.fileName)}</strong><span>${table.rows.length.toLocaleString()} rows · ${table.headers.length} columns</span></div>
      <label class="replace-file" for="file-${kind}">Replace CSV</label>` : `<label class="file-trigger" for="file-${kind}">Choose ${labels[kind].name.toLowerCase()} CSV</label>`}
    <input class="visually-hidden file-input" id="file-${kind}" data-kind="${kind}" type="file" accept=".csv,text/csv" />
...
function bindEvents(): void {
  ...
  document.querySelector<HTMLFormElement>('#restore-form')?.addEventListener('submit', async (event) => { event.preventDefault(); const token = new FormData(event.currentTarget as HTMLFormElement).get('license')?.toString().trim(); if (!token) return; storeToken(token); showNotice('Checking license…'); const verdict = await verifyLicense(true); unlocked = verdict.unlocked; if (!unlocked) clearToken(); render(); showNotice(unlocked ? 'Analyst tools unlocked on this device.' : 'That license is not active. Check the token and try again.'); });
